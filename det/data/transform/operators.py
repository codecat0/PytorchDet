...
from det.data.transform.op_helper import (satisfy_sample_constraint, filter_and_process,
                        ...
                        is_poly, get_border)
from det.modeling.keypoint_utils import affine_transform, get_affine_transform
...
class BaseOperator(object):
    ...
    def apply(self, sample, context=None):
        ...
        对给定的样本进行处理。
        Args:
            sample (dict): 样本的字典形式，例如：{'image': xx, 'label': xxx}
            context (dict, optional): 关于该样本处理的附加信息。默认为None。
        Returns:
            dict: 处理后的样本
        """
        return sample
...
class Decode(BaseOperator):
    ...
    def apply(self, sample, context=None):
        """
        对样本进行处理，若'im_file'字段不为空但'image'为空，则加载图像。
        Args:
            sample (dict): 包含样本信息的字典，包括'im_file'（图像文件路径）和'image'（图像数据）。
            context (Any, optional): 上下文信息，默认为None。此处未使用。
        Returns:
            dict: 更新后的样本信息字典，包含加载后的图像数据以及其他相关信息。
        """
        """
...
class DecodeCache(BaseOperator):
    def __init__(self, cache_root=None):
        ...
        Args:
            cache_root (str, optional): 缓存图像的根目录路径。如果为 None，则不使用缓存。默认为 None。
        """
    ...
    def apply(self, sample, context=None):
        """
        对输入样本进行处理，将其图像数据加载并转换为RGB模式，并更新样本字典。
        Args:
            sample (dict): 包含图像文件路径和可能的其他信息的字典。
            context (Any, optional): 上下文信息，默认为None。
        Returns:
            dict: 更新后的样本字典，包含处理后的图像数据和图像尺寸等信息。
        """
...
class Permute(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        对输入样本进行图像数据转换。
        Args:
            sample (dict): 包含图像数据的字典，其中 'image' 键对应的值为图像数据，类型为 numpy.ndarray。
                            如果样本中包含 'pre_image' 键，则其值也为图像数据，类型为 numpy.ndarray。
            context (Any, optional): 附加上下文信息，默认为 None。此参数目前未使用。
        Returns:
            dict: 转换后的样本字典，其中图像数据已经从 (H, W, C) 格式转换为 (C, H, W) 格式。
        """
        im = sample['image']
...
class Lighting(BaseOperator):
    ...
    def apply(self, sample, context=None):
        """
        将这个扰动加到原图像的每个像素上，实现模拟自然光照变化的效果
        Args:
            sample (dict): 包含图像信息的字典，必须包含 'image' 键，可以包含 'pre_image' 键。
            context (Any, optional): 上下文信息，默认为 None。此参数目前未使用。
        Returns:
            dict: 包含应用噪声后的图像信息的字典。
        """
...
class RandomErasingImage(BaseOperator):
    """
    初始化函数，用于设置随机擦除数据增强的参数。
    https://arxiv.org/abs/1708.04896
    Args:
        prob (float): 随机擦除的执行概率
        lower (float): 擦除区域面积比例的下限
        higher (float): 擦除区域面积比例的上限
        aspect_ratio (float): 擦除区域的宽高比
    """
    ...
    def apply(self, sample, context=None):
        """在每个 ground truth bounding box 内部以一定概率随机擦除一块区域"""
...
class NormalizeImage(BaseOperator):
    def __init__(self,
        ...
        Args:
            mean (list): 像素均值列表。
            std (list): 像素方差列表。
            is_scale (bool): 是否将像素值缩放到[0,1]范围。
            norm_type (str): 归一化类型，取值'mean_std'和'none'。
        """
    ...
    def apply(self, sample, context=None):
        ...
        对图像进行标准化处理。
        Args:
            sample (dict): 包含图像数据和其他信息的字典。
            context (Any, optional): 上下文信息，默认为None。
        Returns:
            dict: 包含标准化后图像数据和其他信息的字典。
        Operators:
            1. (可选) 将像素值缩放到[0,1]范围内。
            2. (可选) 对每个像素值进行均值减去和标准差除以操作。
        """
        im = sample['image']
...
class GridMask(BaseOperator):
    """
    初始化GridMask数据增强类，参考https://arxiv.org/abs/2001.04086
    Args:
        use_h (bool): 是否垂直遮罩
        use_w (bool): 是否水平遮罩
        rotate (float): 遮罩旋转角度
        ...
        ratio (float): 遮罩比率
        mode (int): GridMask模式
        prob (float): 执行GridMask的最大概率
        upper_iter (int): 建议等于全局最大迭代次数max_iter
    """
...
class RandomDistort(BaseOperator):
    """随机颜色失真
    Args:
        hue (list, optional): 色相调整参数，包括最小调整值、最大调整值和步长。默认为[-18, 18, 0.5]。
        saturation (list, optional): 饱和度调整参数，包括最小调整值、最大调整值和步长。默认为[0.5, 1.5, 0.5]。
        contrast (list, optional): 对比度调整参数，包括最小调整值、最大调整值和步长。默认为[0.5, 1.5, 0.5]。
        brightness (list, optional): 亮度调整参数，包括最小调整值、最大调整值和步长。默认为[0.5, 1.5, 0.5]。
        random_apply (bool, optional): 是否随机应用调整。默认为True。
        count (int, optional): 调整的次数。默认为4。
        random_channel (bool, optional): 是否随机选择通道进行调整。默认为False。
        prob (float, optional): 应用调整的概率。默认为1.0。
    """
    ...
    def apply_hue(self, img):
        ...
        为图像添加色调调整效果。
        Args:
            img (PIL.Image.Image): 需要添加色调的图像。
        Returns:
            PIL.Image.Image: 添加了色调调整效果的图像。
        """
    ...
    def apply_saturation(self, img):
        ...
        对图像应用饱和度增强。
        Args:
            img (PIL.Image.Image): 待处理的PIL图像对象。
        Returns:
    ...
    def apply_brightness(self, img):
        ...
        调整图像的亮度。
        Args:
            img (PIL.Image.Image): 输入的图像。
        Returns:
            PIL.Image.Image: 调整亮度后的图像。
        """
    ...
    def apply(self, sample, context=None):
        ...
        对样本进行图像增强处理。
        Args:
            sample (dict): 包含图像数据的字典，其中图像数据以 numpy 数组的形式存储，键为 'image'。
            context (dict, optional): 包含其他信息的字典，默认为 None。
        Returns:
            dict: 包含经过图像增强处理的图像数据的字典。
        """
...
class PhotoMetricDistortion(BaseOperator):
    ...
    Args:
        brightness_delta (int, optional): 亮度变化量，默认值为32。
        contrast_range (tuple, optional): 对比度变化范围，默认值为(0.5, 1.5)。
        saturation_range (tuple, optional): 饱和度变化范围，默认值为(0.5, 1.5)。
        hue_delta (int, optional): 色相变化量，默认值为18。
    """
...
class RandomFlip(BaseOperator):
    ...
    @staticmethod
    def apply_segm(segms, height, width):
    ...
    @staticmethod
    def apply_keypoint(gt_keypoint, width):
    ...
    def apply(self, sample, context=None):
        ...
        对图像和边界框进行翻转。
        Args:
            sample (dict): 包含图像、边界框、分割等信息的数据字典。
            context (None, optional): 未使用的参数，默认为None。
        Operators:
            1. 翻转图像的numpy数组。
            2. 转换边界框的x坐标。
              （必须判断坐标是否归一化！）
...
class Resize(BaseOperator):
    def __init__(self, target_size, keep_ratio, interp=cv2.INTER_LINEAR):
        """
        初始化Resize类，将图像调整到目标大小。
        Args:
            target_size (int|list): 目标大小，可以是整数或列表
            keep_ratio (bool): 是否保持比例，默认为True
            interp (int): 插值方法
        Raises:
            TypeError: 如果target_size的类型不是整数或列表（或元组）时引发
        """
    ...
    def apply_image(self, image, scale):
        ...
        对图像进行缩放处理。
        Args:
            image (numpy.ndarray): 需要缩放的图像，数据类型为numpy数组。
            scale (tuple): 包含两个元素的元组，表示图像在x轴和y轴上的缩放比例。
        Returns:
            numpy.ndarray: 缩放后的图像，数据类型为numpy数组。
        """
    ...
    def apply_bbox(bbox, scale, size):
        ...
        应用边界框缩放和裁剪。
        Args:
            bbox (numpy.ndarray): 形状为 (N, 4) 的边界框数组，每个边界框由 [x_min, y_min, x_max, y_max] 组成。
            scale (tuple): 包含两个元素的元组，表示图像在 x 和 y 方向上的缩放比例。
            size (tuple): 包含两个元素的元组，表示目标尺寸的宽度和高度。
        Returns:
            numpy.ndarray: 经过缩放和裁剪后的边界框数组，形状为 (N, 4)。
        """
        im_scale_x, im_scale_y = scale
    ...
    @staticmethod
    def apply_area(area, scale):
        im_scale_x, im_scale_y = scale
        return area * im_scale_x * im_scale_y

    @staticmethod
    def apply_joints(joints, scale, size):
        """
        对关节点进行缩放和裁剪处理。
        Args:
            joints (numpy.ndarray): 输入的关节点坐标数组，形状为 (N, ..., 2)，其中 N 是关节点的数量，
                                      每个关节点的坐标由两个元素组成，分别表示 x 和 y 坐标。
            scale (tuple): 包含两个元素的元组，分别表示图像在 x 和 y 方向上的缩放比例。
            size (tuple): 包含两个元素的元组，分别表示缩放后图像的新宽度和高度。
        Returns:
            numpy.ndarray: 处理后的关节点坐标数组，形状与输入相同。
        """
        im_scale_x, im_scale_y = scale
        resize_w, resize_h = size
        joints[..., 0] *= im_scale_x
    ...
    def apply_segm(self, segms, im_size, scale):
        ...
        对给定的分割结果进行缩放。
        Args:
            segms (list): 分割结果列表，每个元素可以是多边形列表或RLE编码。
            im_size (tuple): 原始图像的尺寸，格式为 (height, width)。
            scale (tuple): 缩放因子，格式为 (scale_x, scale_x)。
        Returns:
            list: 缩放后的分割结果列表，格式与输入相同。
        """
    ...
    def apply(self, sample, context=None):
        ...
        对图像进行缩放。
        Args:
            sample (dict): 包含图像数据的字典，至少包含'image'键。
            context (Any, optional): 上下文信息，默认为None。
        Returns:
            dict: 更新后的包含缩放后图像的字典。
        Raises:
            TypeError: 如果传入的图像不是numpy数组，则抛出TypeError。
        """
        im = sample['image']
        if not isinstance(im, np.ndarray):
...
class MultiscaleTestResize(BaseOperator):
    def __init__(self,
                 ...
                 use_flip=True):
        """
        将图像缩放至目标尺寸中的每个尺寸，并限制在最大尺寸内
        Args:
            origin_target_size (list): 图像的原始目标尺寸。
            target_size (list): 图像的目标尺寸列表。
            interp (int): 插值方法。
            use_flip (bool): 是否使用翻转增强。
        Raises:
            TypeError: 如果 target_size 或 origin_target_size 的类型不是 List 或 Tuple，则抛出 TypeError 异常。
        """
...
class RandomResize(BaseOperator):
    def __init__(self,
                 ...
                 random_interp=False):
        """
        将图像大小随机调整为目标大小。随机选择目标大小和插值方法
        Args:
            target_size (int, list, tuple): 图像目标大小，如果random_size为True，则必须是list或tuple
            keep_ratio (bool): 是否保持纵横比，默认为True
            interp (int): 插值方法
            random_range (bool): 是否随机选择图像的目标大小，target_size必须是一个[[min_short_edge, long_edge], [max_short_edge, long_edge]]
            random_size (bool): 是否随机选择图像的目标大小
            random_interp (bool): 是否随机选择插值方法
        """
...
class Pad(BaseOperator):
    def __init__(self,
        ...
        初始化Pad类实例。
        Args:
            size (int, Sequence): 目标图像尺寸，如果为None，则填充到size_divisor的倍数，默认为None
            size_divisor (int): 尺寸除数，默认为32
            pad_mode (int): 填充模式，目前仅支持四种模式[-1, 0, 1, 2]。如果为-1，则使用指定的offsets；
                如果为0，则只在右侧和底部填充；如果为1，则根据中心填充；如果为2，则只在左侧和顶部填充
            offsets (list): [offset_x, offset_y]，指定填充时的偏移量，仅支持pad_mode=-1
            fill_value (tuple): 填充区域的RGB值，默认为(127.5, 127.5, 127.5)
        """
    ...
    @staticmethod
    def apply_segm(segms, offsets, im_size, size):
        """
        对给定的分割区域应用偏移量。
        Args:
            segms (list): 分割区域列表，每个元素可以是多边形格式或RLE格式。
            offsets (tuple): 偏移量，格式为(x, y)。
            im_size (tuple): 原始图像尺寸，格式为(height, width)。
            size (tuple): 目标图像尺寸，格式为(h, w)。
        Returns:
            list: 应用偏移量后的分割区域列表。
        """
    ...
    def apply_bbox(bbox, offsets):
        ...
        对边界框应用偏移量。
        Args:
            bbox (list): 边界框坐标，格式为 [x_min, y_min, x_max, y_max]。
            offsets (list): 偏移量，格式为 [dx, dy]。
        Returns:
            list: 应用偏移量后的边界框坐标，格式为 [x_min, y_min, x_max, y_max]。
        """
        return bbox + np.array(offsets * 2, dtype=np.float32)

    @staticmethod
    def apply_keypoint(keypoints, offsets):
        """
        将给定的偏移量应用到关键点坐标上。
        Args:
            keypoints (list of list of float): 包含关键点的列表，每个关键点由一对浮点数表示（x, y）。
            offsets (list of float): 包含偏移量的列表，偏移量的数量应与关键点数量一致。
        Returns:
            list of list of float: 应用偏移量后的关键点列表。
        """
        n = len(keypoints[0]) // 2
    ...
    def apply_image(self, image, offsets, im_size, size):
        ...
        对图像应用偏移量。
        Args:
            image (numpy.ndarray): 需要应用偏移量的图像数组。
            offsets (tuple): 偏移量，格式为(x, y)。
            im_size (tuple): 原始图像尺寸，格式为(height, width)。
            size (tuple): 目标图像尺寸，格式为(h, w)。
        Returns:
            canvas (numpy.ndarray): 偏移后的图像数组。
        """
        x, y = offsets
        im_h, im_w = im_size
        h, w = size
...
class RandomExpand(BaseOperator):
    ...
    def __init__(self, ratio=4., prob=0.5, fill_value=(127.5, 127.5, 127.5)):
        ...
        初始化函数。
        Args:
            ratio (float, optional): 扩展比例，默认为4.0。表示原图将扩展到的倍数。必须大于1.01。
            prob (float, optional): 扩展概率，默认为0.5。表示对图像进行扩展的概率。
            fill_value (tuple, optional): 扩展区域的填充值，默认为(127.5, 127.5, 127.5)。
                可以是单个浮点数，表示RGB三个通道使用相同的填充值；也可以是长度为3的元组，
                分别表示R、G、B三个通道的填充值。
        Raises:
            AssertionError: 如果ratio不大于1.01，或者fill_value不是数字或序列类型，将引发断言错误。
        """
    ...
    def apply(self, sample, context=None):
        ...
        对输入样本进行随机偏移
        Args:
            sample (dict): 输入样本，包含图像数据和其他信息。
            context (dict, optional): 包含上下文信息，默认为None。
        Returns:
            dict: 处理后的样本。
        """
        if np.random.uniform(0., 1.) < self.prob:
...
class CropWithSampling(BaseOperator):
    def __init__(self, batch_sampler, satisfy_all=False, avoid_no_bbox=True):
        """
        初始化裁剪采样器，配置裁剪参数和采样策略。
        Args:
            batch_sampler (list): 包含多组不同裁剪参数的列表，
            参数格式: [最大采样数, 最大尝试次数, 最小比例, 最大比例,
                      最小宽高比, 最大宽高比, 最小重叠度, 最大重叠度]
            ...
            satisfy_all (bool): 是否要求所有边界框都满足裁剪条件。
            avoid_no_bbox (bool): 是否避免出现边界框不出现的情况。
        """
    ...
    def apply(self, sample, context=None):
        ...
        操作步骤：
            ...
            4. 判断新边界框是否在新图像中有效。
        Args:
            sample (dict): 包含图像和标注信息的字典，必须包含'image'键。
                            可选包含'gt_bbox'、'gt_class'和'gt_score'键。
            context (dict, optional): 上下文信息，默认为None。
        Returns:
            dict: 返回更新后的样本字典，包含裁剪后的图像和调整后的边界框。
        Raises:
            AssertionError: 当输入样本中不包含'image'键时抛出。
        """
...
class CropWithDataAchorSampling(BaseOperator):
    def __init__(self,
                 ...
                 avoid_no_bbox=True):
        """
        初始化CropWithDataAnchorSampling类，用于带数据锚点采样的图像裁剪。
        Args:
            batch_sampler (list): 多组不同的裁剪参数集合。
                示例: [[1, 10, 1.0, 1.0, 1.0, 1.0, 0.0, 0.0, 0.2, 0.0]]
                ...
                参数格式: [最大采样数, 最大尝试次数, 最小比例, 最大比例,
                          最小宽高比, 最大宽高比,
                          最小重叠度, 最大重叠度, 最小覆盖率, 最大覆盖率]
            anchor_sampler (list, optional): 用于锚点采样的不同参数集合。
            target_size (int, optional): 目标图像尺寸。
            das_anchor_scales (list[float]): 数据锚点采样中使用的锚点尺度列表。
            sampling_prob (float): 采样概率，默认为0.5。
            min_size (float): 采样边界框的最小尺寸，默认为8.0。
            avoid_no_bbox (bool): 是否避免出现没有边界框的情况，默认为True。
        """
    ...
    def apply(self, sample, context=None):
        ...
        操作步骤：
            ...
            4. 判断新边界框在新图像中是否满足条件。
        处理逻辑：
            - 根据采样概率决定使用锚点采样(anchor sampling)还是批量采样(batch sampling)。
            - 锚点采样：从预设的锚点比例中随机选择满足条件的裁剪区域。
            - 批量采样：从预设的批量比例中随机选择满足条件的裁剪区域。
            - 对选定的裁剪区域进行过滤处理，包括边界框、类别、分数和关键点的调整。
            - 根据目标尺寸和最小尺寸对边界框进行面积采样。
            - 如果避免无边界框(avoid_no_bbox)为真，则跳过不包含边界框的裁剪。
        Args:
            self: 类实例，包含采样参数和配置。
            sample: 字典类型，包含以下键值：
                - 'image': 原始图像数据。
                ...
                - 'gt_class': 真实类别标签。
                - 'gt_score': (可选)真实分数。
                - 'gt_keypoint': (可选)真实关键点坐标。
                - 'keypoint_ignore': (可选)忽略的关键点。
            context: (可选)上下文信息，当前未使用。
        Returns:
            dict: 更新后的样本字典，包含以下更新：
                - 'image': 裁剪后的图像数据。
                - 'gt_bbox': 调整后的边界框坐标。
                - 'gt_class': 调整后的类别标签。
                - 'gt_score': (可选)调整后的分数。
                - 'gt_keypoint': (可选)调整后的关键点坐标。
                - 'keypoint_ignore': (可选)调整后的忽略关键点。
        """
        assert 'image' in sample, "image data not found"
        im = sample['image']
        gt_bbox = sample['gt_bbox']
...
class RandomCrop(BaseOperator):
    def __init__(self,
        ...
        初始化随机裁剪操作的配置参数。
        Args:
            aspect_ratio (list, optional): 允许的宽高比范围，默认为[.5, 2.]。
            thresholds (list, optional): 用于裁剪的IOU阈值列表，默认为[.0, .1, .3, .5, .7, .9]。
            scaling (list, optional): 允许的缩放比例范围，默认为[.3, 1.]。
            num_attempts (int, optional): 尝试生成裁剪区域的次数，默认为50。
            allow_no_crop (bool, optional): 是否允许不进行裁剪，默认为True。
            cover_all_box (bool, optional): 是否强制覆盖所有边界框，默认为False。
            is_mask_crop (bool, optional): 是否对掩码进行裁剪，默认为False。
            ioumode (str, optional): 计算IOU的模式，默认为"iou"。
            prob (float, optional): 执行随机裁剪的概率，默认为1.0。
        """
    ...
    def crop_segms(self, segms, valid_ids, crop, height, width):
        """
        对分割掩码进行裁剪操作，支持多边形(Polygon)和RLE(Run Length Encoding)两种格式。
        该函数主要处理两种格式的分割掩码：
        1. 多边形格式：使用shapely库进行几何运算，处理多边形与裁剪区域的交集
        2. RLE格式：使用pycocotools库进行掩码的解码、裁剪和重新编码
        Args:
            segms: 原始分割掩码列表，每个元素可以是多边形坐标或RLE格式
            valid_ids: 需要处理的有效掩码ID列表
            crop: 裁剪区域坐标，格式为[xmin, ymin, xmax, ymax]
            height: 原始图像高度
            width: 原始图像宽度
        Returns:
            裁剪后的分割掩码列表，所有结果都转换为多边形格式
        内部方法:
            _crop_poly: 处理多边形格式掩码的裁剪
            _crop_rle: 处理RLE格式掩码的裁剪
        """
    ...
    def polygon_to_rle(self, polygons, height, width):
        """
        将多边形坐标转换为RLE（Run-Length Encoding）格式掩码
        首先创建一个指定尺寸的空白掩码图像，然后将输入的多边形区域填充为白色，
        最后将二进制掩码转换为RLE编码格式。
        Args:
            polygons: 多边形坐标列表，每个多边形由顶点坐标组成
            height: 掩码图像的高度
            width: 掩码图像的宽度
        Returns:
            RLE编码格式的掩码数据
    ...
    def rle_to_polygon(self, rle_mask, min_area=5):
        """
        将RLE格式的掩码转换为多边形坐标列表。
        该函数首先解码RLE掩码为二值掩码，然后通过查找轮廓将二值掩码转换为多边形坐标，
        并过滤掉面积小于阈值的多边形。
        Args:
            rle_mask: RLE格式的掩码数据。
            min_area: 过滤多边形的最小面积阈值，默认为5。
        Returns:
            多边形坐标列表，每个多边形由连续的x,y坐标点组成（如[x1,y1,x2,y2,...]）。
        """
    ...
    def set_fake_bboxes(self, sample):
        """
        为样本数据设置假的边界框和类别标签。
        该函数会向输入的sample字典中添加两个键值对：
        - 'gt_bbox': 包含10个假边界框坐标的numpy数组，每个边界框格式为[x1, y1, x2, y2]
        - 'gt_class': 包含10个假类别标签的numpy数组，每个标签为1-10的整数
        Args:
            sample: 字典类型，包含需要添加假标注的样本数据
        Returns:
            更新后的样本字典，包含假边界框和类别标签
        """
    ...
    def apply(self, sample, context=None):
        """
        根据概率决定是否对样本应用随机裁剪变换。
        如果随机数大于预设概率，则直接返回原始样本。
        对于无真实标注框的样本（如半监督场景下的无监督数据），会生成伪标注框后进行裁剪，
        并删除伪标注框和类别信息。对于有标注框但为空的样本，直接返回。
        其他情况则对样本进行随机裁剪。
        Args:
            sample: 输入样本数据，通常包含图像和标注信息。
            context: 可选上下文信息，当前未使用。
        Returns:
            处理后的样本数据，可能包含裁剪后的图像和更新后的标注信息。
        """
        if random.random() > self.prob:
            return sample
    ...
    def random_crop(self, sample, fake_bboxes=False):
        """
        对输入样本进行随机裁剪操作，根据不同的阈值尝试生成有效的裁剪框。
        该函数会随机选择一个阈值（或'no_crop'选项），然后尝试生成满足条件的裁剪框。
        如果选择'no_crop'则直接返回原样本；否则会尝试多次生成裁剪框，直到找到满足
        与真实框(IoU/IoF)阈值条件的裁剪框，或遍历完所有尝试次数。
        Args:
            sample (dict): 包含图像和标注信息的字典，必须包含'image'和'gt_bbox'键。
                            可选包含'gt_poly', 'gt_segm', 'gt_class', 'gt_score',
                            'is_crowd', 'difficult', 'gt_joints'等键。
            fake_bboxes (bool): 是否返回假边界框（不更新gt_bbox等标注信息）。
                                如果为True，则只裁剪图像不更新标注信息。
        Returns:
            dict: 裁剪后的样本字典，包含裁剪后的图像和更新后的标注信息。
                  如果未找到有效裁剪框或选择'no_crop'，则返回原始样本。
        """
    ...
    def _iou_matrix(self, a, b):
        ...
        Args:
            a: 形状为(N,4)的numpy数组，表示第一组边界框，格式为[x1,y1,x2,y2]
            b: 形状为(M,4)的numpy数组，表示第二组边界框，格式为[x1,y1,x2,y2]
        Returns:
            形状为(N,M)的numpy数组，表示a中每个框与b中每个框的IoU值
        """
    ...
    def _gtcropiou_matrix(self, a, b):
        """
        计算矩阵a中每个矩形与矩阵b中每个矩形的交并比(IoU)的交集部分占比。
        Args:
            a: 形状为(N,4)的numpy数组，表示N个矩形，每个矩形由[x1,y1,x2,y2]坐标定义。
            b: 形状为(M,4)的numpy数组，表示M个矩形，每个矩形由[x1,y1,x2,y2]坐标定义。
        Returns:
            形状为(N,M)的numpy数组，表示a中每个矩形与b中每个矩形的交集面积除以a中矩形面积的比例。
            添加了1e-10的平滑项以避免除以零。
        """
        tl_i = np.maximum(a[:, np.newaxis, :2], b[:, :2])
        br_i = np.minimum(a[:, np.newaxis, 2:], b[:, 2:])
    ...
    def _crop_box_with_center_constraint(self, box, crop):
        """
        根据裁剪区域和中心点约束裁剪边界框。
        该函数首先将边界框限制在裁剪区域内，然后计算原始边界框的中心点，
        筛选出中心点位于裁剪区域内且裁剪后宽度/高度仍为正的边界框。
        Args:
            box: 原始边界框数组，形状为(N,4)，格式为[x1,y1,x2,y2]
            crop: 裁剪区域数组，形状为(4,)，格式为[x1,y1,x2,y2]
        Returns:
            tuple: 包含两个元素的元组:
                - 裁剪后的边界框数组，形状与输入box相同
                - 有效边界框的索引数组
        """
...
class RandomShortSideResize(BaseOperator):
    ...
    @staticmethod
    def get_size_with_aspect_ratio(image_shape, size, max_size=None):
        h, w = image_shape
        max_clip = False
...
class RandomShift(BaseOperator):
    ...
    @staticmethod
    def calc_shift_coor(im_h, im_w, shift_h, shift_w):
        return [
