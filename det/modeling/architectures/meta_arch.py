...
from det.modeling.post_process import nms
...
class BaseArch(nn.Module):
    ...
    def load_meanstd(self, cfg_transform):
        ...
        if self.data_format == 'NHWC':
            self.register_buffer('scale', torch.tensor(scale / std).reshape((1, 1, 1, 3)))
            self.register_buffer('bias', torch.tensor(-mean / std).reshape((1, 1, 1, 3)))
        else:
            self.register_buffer('scale', torch.tensor(scale / std).reshape((1, 3, 1, 1)))
            self.register_buffer('bias', torch.tensor(-mean / std).reshape((1, 3, 1, 1)))
    ...
    @staticmethod
    def build_inputs(data, input_def):
        inputs = {}
