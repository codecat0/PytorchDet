...
from det.modeling.backbones.name_adapter import NameAdapter
from det.modeling.shape_spec import ShapeSpec
...
class ConvNormLayer(nn.Module):
    def __init__(self,
        ...
        if not self.dcn_v2:
            ...
            self.conv_offset = nn.Conv2d(
                ...
                padding=(filter_size - 1) // 2,
                bias=True)
            # Initialize offset and mask layers to zero
            with torch.no_grad():
    ...
    def forward(self, inputs):
        if not self.dcn_v2:
            ...
            out = self.conv(inputs, offset, mask)

        if self.norm_type in ['bn', 'sync_bn']:
            out = self.norm(out)
        if self.act:
            out = getattr(F, self.act)(out) if hasattr(F, self.act) else F.relu(out)
        return out
...
class SELayer(nn.Module):
    def __init__(self, ch, reduction_ratio=16):
        ...
        self.squeeze = nn.Linear(ch, c_)
        self.extract = nn.Linear(c_, ch)
...
class Blocks(nn.Module):
    def __init__(self,
        ...
        for i in range(count):
            ...
            self.add_module(conv_name, block(
                ch_in=ch_in,
                ch_out=ch_out,
            ...
            self.blocks.append(self.get_submodule(conv_name))
            if i == 0:
...
class ResNet(nn.Module):
    def __init__(self,
        ...
        for i in range(num_stages):
            ...
            self.add_module(res_name, Blocks(
                block,
                self.ch_in,
                ch_out_list[i],
                count=block_nums[i],
            ...
            # setattr(self, res_name, res_layer)
            self.res_layers.append(self.get_submodule(res_name))
